...
interface LoadingForm {
    ...
    clickable?: Function;
    /** 알림 배경색상, rrggbb로 구성된 문자열 */
    bgcolor?: string;
    /** 추가 소리알림 사용 여부 */
    soundAlert?: boolean;
}

export class P5Loading {
    constructor() { }
                ...
                this.CurrentLoading = new p5((p: p5) => {
                    p.setup = () => {
                        ...
                        this.AddLoadingInfo = (info: LoadingForm) => {
                            ...
                            thisLoading.class('backdrop_filter');
                            thisLoading.style("width: 100%; height: fit-content");
                            thisLoading.style("word-break: break-all");
                            thisLoading.style(`background: ${info.bgcolor ? `#${info.bgcolor}88` : 'var(--toast-background-color)'}`);
                            ...
                            if (info.clickable) {
                                ...
                                thisLoading.style("pointer-events: auto");
                            ...
                            if (info.clickable !== undefined || info.soundAlert) this.SoundAlert?.();
                            // 레이아웃용 flex-div
                            ...
                            // 지정된 텍스트
                            const message = p.createDiv(info.message || '잠시만 기다려주세요');
    ...
    async update(info: LoadingForm, only_update = false) {
        if (this.stackKeys.includes(info.id)) {
            ...
            if (info.clickable) {
                ...
                this.loadingStack[info.id].element.style("pointer-events: auto");
                this.loadingStack[info.id].element.mouseClicked(() => {
                    ...
                    this.loadingStack[info.id].forceEnd = 350;
                });
                // 클릭 가능한 알림이 발생하면 설정에 따라 소리알림을 발생
            } else {
                this.loadingStack[info.id].element.style("cursor: auto");
                this.loadingStack[info.id].element.style("pointer-events: none");
            }
            ...
            if (info.bgcolor !== undefined) {
                if (info.bgcolor !== null)
                    this.loadingStack[info.id].element.style(`background: ${info.bgcolor ? `#${info.bgcolor}88` : 'var(--toast-background-color)'}`);
            }
            // 이 알림은 곧 종료됩니다
...
export const p5loading = new P5Loading();